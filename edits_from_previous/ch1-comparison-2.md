...
| 1.3.1.2 Use Lists in Loops | Index-based `range()` loops vs. `enumerate()` | **Kept** — `enumerate` is core in new 1.2 (Exercise 5: "Pair two stations with zip" extends the same idea to `zip`, which isn't confirmed in the old material). |
...
| 1.3.4.1 Functions | `def`, optional/default arguments, `*args`, lambda, `map()`/`filter()` | **Reduced, then partially restored.** `def`, default arguments, and multi-return (tuple unpacking) are core in new 1.2. `*args` was initially dropped with no equivalent — **now restored as core**, right after default arguments, with its own worked example (`mean_of(*values)`) and a dedicated Exercise 8 ("A variable number of arguments") plus solution. `map()`/`filter()` are still dropped from core; new 1.2's Exercise 12 reaches a filter-like result via a list comprehension instead of the `filter()` builtin. Lambda is demoted to a going-deeper box ("lambda and scope (LEGB)"). |
...
| Old 1.4, Warm-up | Convert a list comprehension to an explicit loop; generate the first 20 cubes | Not reproduced as such; comprehensions are now going-deeper material with their own optional exercise set (10–14) instead of a core warm-up. |
...
- `*args` restored as core content (see the row above), with its own worked example and
  Exercise 8.
- Exercise 13 (spot the impure function) and Exercise 14 (reject impossible temperatures via
  `raise`).
