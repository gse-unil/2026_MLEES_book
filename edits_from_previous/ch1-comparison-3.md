...
| *(no old equivalent)* | — | **New:** `np.where`; NaN-aware reductions (`np.nanmean`) vs. ordinary ones; `np.interp` for 1D gap-filling; a going-deeper box on basic linear algebra (`np.linalg.solve`); a going-deeper box on dask for bigger-than-memory arrays. None of these are confirmed anywhere in the old material — old 1.5's outline shows no missing-data, interpolation, or linear-algebra content at all. Boolean masking itself is *not* new: the old tutorial notebook (`W2_S1_Tutorial.ipynb`) already has a brief boolean-indexing example (`idx = xx<0; yy[idx].shape`); new 1.3 formalizes and expands this into its own named topic with a mermaid diagram and a worked freezing-cells example. |
